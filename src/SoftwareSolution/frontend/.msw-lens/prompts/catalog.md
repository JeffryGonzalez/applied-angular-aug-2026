# msw-lens context
generated: 2026-08-26T23:07:58.890Z
entry: src/app/areas/catalog/feature-catalog/pages/catalog.ts

---

## The ask

I'm working on the `Catalog` component in a web application and want to
...
@Component({
  ...
  template: `
    ...
    <table class="table">
      ...
      <tbody>
        @for (item of store.catalogResourse.value(); track item.id) {
          <tr>
            <td>{{ item.id }}</td>
            <td>{{ item.name }}</td>
            <td>{{ item.vendorId }}</td>
            <td>{{ item.isDeprecated }}</td>
          </tr>
        }
      </tbody>
    </table>
...
import { z } from 'zod';
import { zCatalogCreateModel } from '../../shared/api/zod.gen';
